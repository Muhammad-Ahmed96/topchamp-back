class EventContestCategoryBracketDetail < ApplicationRecord
  ...
  scope :skill_filter, lambda {|skill| where("event_contest_category_bracket_details.lowest_skill <= ?", skill).where("event_contest_category_bracket_details.highest_skill >= ?", skill)
